...
class ApiGateway {
    ...
    constructor({ routes }: { routes: Array<any> }) {
        this.api = new aws.apigatewayv2.Api("que-pelicula-ver-api", {
            protocolType: "HTTP",
            description: "Que pelicula Ver API - API Gateway"
        });
        ...
        for (let i = 0; i < routes.length; i++) {
            ...
            const integration = new aws.apigatewayv2.Integration(route.name, {
                description: "Que pelicula Ver API - GraphQL API Integration",
                apiId: this.api.id,
