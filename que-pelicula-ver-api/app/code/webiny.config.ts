...
// Within both commands, we rely on the deployed GraphQL API (que-pelicula-ver-api/api) and its stack
...
// - `que-pelicula-ver-api/api/pulumi/dev/index.ts`
// - `que-pelicula-ver-api/api/pulumi/prod/index.ts`
    ...
    commands: {
        async watch(options, context) {
            ...
            Object.assign(
                process.env,
                getStackOutput({
                    folder: "que-pelicula-ver-api/api",
                    env: options.env,
                    map: API_MAP
                })
            );
